...
# Support JWT token for OAuth 2.1
 ...
 # Replace token from policy with actual value from JWT
                ...
                print('filterPred = ' + filterPred + ' queryKey = ' + queryKey + " queryPred = " + queryPred)
                jsonRet = sqlSetup(ans, filterPred, queryPred)
                print('jsonRet = ' + str(jsonRet))
                r = cleanReturn(json.dumps(jsonRet), returnHeaders)
                return r
  #              return(json.dumps(jsonRet), VALID_RETURN)
...
def logToKafka(jString, kafka_topic):
    ...
    try:
        ...
        print("Write to Kafka failed.  Is the server on " + KAFKA_SERVER + " running?")
        print(e)
        kafkaDisabled = True

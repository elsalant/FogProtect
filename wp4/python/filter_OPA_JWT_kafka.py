...
def connectKafka():
    global kafkaWaitingFirstConnect
    global kafkaDisabled
# Set up Kafka connection
    if (kafkaDisabled == False and kafkaWaitingFirstConnect):
        global producer
        try:
            ...
            print(e)
            kafkaDisabled = True
        kafkaWaitingFirstConnect = False
    ...
    # Determine if the requester has access to this URL.  If the requested endpoint shows up in blockDict, then return 500
        ...
        for resultDict in blockDict['result']:
            ...
            if blockURL == "BlockURL":
                logToKafka(jString, KAFKA_DENY_TOPIC)
                return ("Access denied!", ACCESS_DENIED_CODE)
            else:
                logToKafka(jString, KAFKA_ALLOW_TOPIC)
...
def logToKafka(jString, kafka_topic):
    global kafkaDisabled
    if kafkaWaitingFirstConnect == True:
        connectKafka()
    ...
    jSONoutBytes = str.encode(jSONout)
    try:
        print("Writing to Kafka queue " + kafka_topic + ": " + jSONout)

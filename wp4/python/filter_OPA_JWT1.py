...
REDIRECT_SERVICE = os.getenv("QUERYGW_URI") if os.getenv("QUERYGW_URI") else "querygw"
REDIRECT_PORT = 9005
...
CUSTOM_FILTER = os.getenv("COLUMN_FILTER") if os.getenv("COLUMN_FILTER") else None
...
# Support JWT token for OAuth 2.1
    ...
    print("headersList = " + str(headersList))
    if (payloadEncrypted != None):
        noJWT = False
        roleKey = os.getenv("SCHEMA_ROLE") if os.getenv("SCHEMA_ROLE") else FIXED_SCHEMA_ROLE
        try:
            role = decryptJWT(payloadEncrypted, roleKey)  #comes back as a Python dictionary
        except:
 ...
 # The input can actually be a list, a string (JSON), or interpreted by Python to be a dict
    ...
    while processing:
        processing = False
        if (type(ans) is str):
            if (returnHeaders['Content-Type'] == 'video/mp4'):
            ...
            if (type(ans[listIndex]) is not str):
                print("---> WARNING: list of " + str(type(ans[listIndex])) + " - not filtering")
                return(str(ans), VALID_RETURN)
            jsonDict = json.loads(ans[listIndex])
            listIndex += 1
  ...
  #  for line in ans:
        for resultDict in filterDict['result']:
            ...
            if (action == 'Allow'):
                r = cleanReturn(json.dumps(ans), request.headers)
  ...
  #              return (json.dumps(ans), VALID_RETURN)
    ...
    r = cleanReturn(json.dumps(filteredLine), request.headers)
    return r, VALID_RETURN
 #   return (json.dumps(filteredLine), VALID_RETURN)
...
def recurse(jDict, keySearch, action):
    try:
        ...
        print("Error on keySearch.pop(0), keySearch = ", str(keySearch))
        return(jDict)
    while i in jDict:
        last = jDict[i]
        if not keySearch:
